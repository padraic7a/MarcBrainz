...
for barcode in barcodes:
    ...
    if 'releases' in data and data['releases']:
        release = data['releases'][0] # Assuming you want information about the first matching release
        ...
        marc_record.add_field(Field(tag='245', indicators=['0', '0'], subfields = [Subfield(code='a', value= title)]))

        # Add artist field (tag 100)
        artist_name = release['artist-credit'][0]['artist']['name']
        marc_record.add_field(Field(tag='100', indicators=['1', ' '], subfields = [Subfield(code='a', value= artist_name)]))
        
        # Add year field (tag 260)
        marc_record.add_field(Field(tag='260', indicators=[' ', ' '], subfields= [Subfield(code='c', value= release['date'])]))

        # Add track info field (tag 500)
        track_info = ', '.join([f"{track['position']}. {track['title']}" for track in tracklist_data['media'][0]['tracks']])
        marc_record.add_field(Field(tag='500', indicators=[' ', ' '], subfields= [Subfield(code='a', value= track_info)]))
        ...
        csv_rows.append([release['title'], artist_name, release['date'], track_info, release['id']])
    else:
        # Create a MARC record for 'Not found'
        marc_record = Record()
        marc_record.add_field(Field(tag='245', indicators=['0', '0'], subfields= [Subfield(code='a', value= 'Not found')]))
        marc_records.append(marc_record)
...
with open(csv_file, 'w', newline='', encoding='utf-8') as csvfile:
    ...
    csvwriter.writerow(['Title', 'Artist', 'Release Date', 'Track Info', 'ID']) # CSV header
